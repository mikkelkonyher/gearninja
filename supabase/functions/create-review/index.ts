...
serve(async (req) => {
  ...
  try {
    ...
    if (!sale_id) {
      return badRequest("sale_id er påkrævet");
    }

    if (!rating || typeof rating !== "number" || rating < 1 || rating > 5) {
      return badRequest("Bedømmelse skal være mellem 1 og 5");
    }

    // Validate content length (max 1000 characters)
    if (content && content.length > 1000) {
      return badRequest("Kommentar må maks være 1000 tegn");
    }
    ...
    if (saleError || !sale) {
      return jsonResponse({ error: "Salg ikke fundet" }, { status: 404 });
    }

    // Verify user is part of the sale
    if (user.id !== sale.seller_id && user.id !== sale.buyer_id) {
      return jsonResponse({ error: "Du er ikke en del af dette salg" }, { status: 403 });
    }
    ...
    if (reviewee_id === user.id) {
      return badRequest("Du kan ikke anmelde dig selv");
    }
    ...
    if (reviewError) {
      ...
      return jsonResponse({ error: "Kunne ikke oprette anmeldelse" }, { status: 500 });
    }
